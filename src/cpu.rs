...
impl Cpu {
        ...
        let tmp = match ops.len {
             ...
             2 => {
                 ...
                 match ops.mode {
                     ...
                     AddressingMode::IndirectX => format!("(${:02X},X) @ {:02X} = {:04X} = {:02X}", address, (address.wrapping_add(self.x)), mem_addr, stored_value),
                     AddressingMode::IndirectY => format!("(${:02X}),Y = {:04X} @ {:04X} = {:02X}", address, (mem_addr.wrapping_sub(self.y as u16)), mem_addr, stored_value),
    ...
    pub fn step(&mut self, bus: &mut Bus) -> u16 {
        ...
        match opcode {
            ...
            0xA1 => { self.lda(bus, AddressingMode::IndirectX); }
            0xB1 => { self.lda(bus, AddressingMode::IndirectY); }
            ...
            0x81 => { self.sta(bus, AddressingMode::IndirectX); }
            0x91 => { self.sta(bus, AddressingMode::IndirectY); }
            ...
            0x21 => { self.and(bus, AddressingMode::IndirectX); }
            0x31 => { self.and(bus, AddressingMode::IndirectY); }
            ...
            0x01 => { self.ora(bus, AddressingMode::IndirectX); }
            0x11 => { self.ora(bus, AddressingMode::IndirectY); }
            ...
            0x41 => { self.eor(bus, AddressingMode::IndirectX); }
            0x51 => { self.eor(bus, AddressingMode::IndirectY); }
            ...
            0xC1 => { self.cmp(bus, AddressingMode::IndirectX); }
            0xD1 => { self.cmp(bus, AddressingMode::IndirectY); }
            ...
            0x61 => { self.adc(bus, AddressingMode::IndirectX); }
            0x71 => { self.adc(bus, AddressingMode::IndirectY); }
            ...
            0xE1 => { self.sbc(bus, AddressingMode::IndirectX); }
            0xF1 => { self.sbc(bus, AddressingMode::IndirectY); }

            // Jumps and Calls
            0x4C => { self.jmp(bus, AddressingMode::Absolute); }
            0x6C => { self.jmp(bus, AddressingMode::Indirect); } // Need Indirect mode for JMP
            0x20 => { self.jsr(bus); }
            0x60 => { self.rts(bus); }
            ...
            0x02 | 0x12 | 0x22 | 0x32 | 0x42 | 0x52 | 0x62 | 0x72 | 0x92 | 0xB2 | 0xD2 | 0xF2 => { 
                ...
                match mode {
                    AddressingMode::Immediate => { self.pc = self.pc.wrapping_add(1); }
                     _ => {
            ...
            0x03 => { self.slo(bus, AddressingMode::IndirectX); }
            0x13 => { self.slo(bus, AddressingMode::IndirectY); }
            ...
            0x23 => { self.rla(bus, AddressingMode::IndirectX); }
            0x33 => { self.rla(bus, AddressingMode::IndirectY); }
            ...
            0x43 => { self.sre(bus, AddressingMode::IndirectX); }
            0x53 => { self.sre(bus, AddressingMode::IndirectY); }
            ...
            0x63 => { self.rra(bus, AddressingMode::IndirectX); }
            0x73 => { self.rra(bus, AddressingMode::IndirectY); }
            ...
            0x83 => { self.sax(bus, AddressingMode::IndirectX); }
            ...
            0xA3 => { self.lax(bus, AddressingMode::IndirectX); }
            0xB3 => { self.lax(bus, AddressingMode::IndirectY); }
            ...
            0xC3 => { self.dcp(bus, AddressingMode::IndirectX); }
            0xD3 => { self.dcp(bus, AddressingMode::IndirectY); }
            ...
            0xE3 => { self.isc(bus, AddressingMode::IndirectX); }
            0xF3 => { self.isc(bus, AddressingMode::IndirectY); }
        ...
        let cycles = crate::opcodes::OPCODES_MAP.get(&opcode).map(|op| op.cycles).unwrap_or(0);
    ...
    // Status Flags
    fn clc(&mut self) { self.st &= !0x01; }
    fn sec(&mut self) { self.st |= 0x01; }
    fn cli(&mut self) { self.st &= !0x04; }
    fn sei(&mut self) { self.st |= 0x04; }
    fn clv(&mut self) { self.st &= !0x40; }
    fn cld(&mut self) { self.st &= !0x08; }
    fn sed(&mut self) { self.st |= 0x08; }
