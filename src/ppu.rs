...
impl Ppu {
    ...
    pub fn tick(&mut self, cycles: u16) -> bool {
        ...
        for _ in 0..cycles {
            ...
            // NMI edge detection: fire NMI only on rising edge (false→true)
            let new_nmi_output = (self.status & 0x80) != 0 && (self.ctrl & 0x80) != 0;
            if new_nmi_output && !self.nmi_output {
                nmi_triggered = true;
                // Also set persistent flag so NMI from PPU catch-up isn't lost
    ...
    fn write_ctrl(&mut self, data: u8) {
        ...
        // NMI edge detection: check if enabling NMI while VBlank is active
        // creates a rising edge on the NMI output
        let new_nmi_output = (self.status & 0x80) != 0 && (self.ctrl & 0x80) != 0;
        if new_nmi_output && !self.nmi_output {
            self.nmi_interrupt = true;
        }
    ...
    fn read_status(&mut self) -> u8 {
        ...
        // Update NMI output (VBlank cleared → NMI output goes low)
        ...
        match addr {
            0x0000..=0x1FFF => {
                ...
                if self.chr_rom.len() > 0 {
                    if self.mapper == 1 {
                        let offset = self.mmc1_chr_offset(addr as usize, self.chr_rom.len());
                        self.chr_rom[offset]
                    } else if self.mapper == 3 {
                        let bank_size = 8192; // 8KB
                        let num_banks = self.chr_rom.len() / bank_size;
                        let target_bank = if num_banks > 0 {
                            self.chr_bank % num_banks
                        } else {
                            0
                        };
    ...
    fn mirror_vram_addr(&self, addr: u16) -> u16 {
        ...
        match (self.mirroring, table) {
            ...
            (Mirroring::OneScreenLower, _) => offset,
            (Mirroring::OneScreenUpper, _) => 0x400 + offset,

            _ => addr & 0x7FF, // Fallback?
        }
    }

    fn mmc1_chr_offset(&self, addr: usize, total_len: usize) -> usize {
        if total_len == 0 {
            return 0;
        }

        let chr_mode_4k = (self.mmc1_control & 0x10) != 0;

        if chr_mode_4k {
            let bank_size = 0x1000;
            let num_banks = (total_len / bank_size).max(1);
            if addr < bank_size {
                let bank = (self.mmc1_chr_bank0 as usize) % num_banks;
                (bank * bank_size + addr) % total_len
            } else {
                let bank = (self.mmc1_chr_bank1 as usize) % num_banks;
                (bank * bank_size + (addr - bank_size)) % total_len
            }
        } else {
    ...
    fn update_shifters(&mut self) {
        if (self.mask & 0x18) != 0 {
            // If rendering enabled (BG or Sprite)
    ...
    fn render_pixel(&mut self) {
        ...
        if mask_bg {
            if mask_bg_left || self.cycle > 8 {
                ...
                let pal0 = if (self.bg_shifter_attrib_lo & bit_mux) != 0 {
                    1
                } else {
                    0
                };
                let pal1 = if (self.bg_shifter_attrib_hi & bit_mux) != 0 {
                    2
                } else {
                    0
                };
        ...
        if mask_spr {
            if mask_spr_left || self.cycle > 8 {
                for i in 0..self.sprite_count {
                    if self.sprite_latch_x[i as usize] == 0 {
                        let p0 = if (self.sprite_shifter_pattern_lo[i as usize] & 0x80) != 0 {
                            1
                        } else {
                            0
                        };
                        let p1 = if (self.sprite_shifter_pattern_hi[i as usize] & 0x80) != 0 {
                            2
                        } else {
                            0
                        };
